...
parser.add_argument('--max_samples', type=int, default=29, help='mFTEMaximum samples to train')
...
if __name__ == "__main__":
    ...
    model_2 = net_factory(net_type='taylorecho', in_chns=1, class_num=num_classes - 1, mode="train")
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            loss_seg_1 = CE(y_prob_1.squeeze(1), labeled_gt_set.float())
            loss_seg_dice_1 = dice_loss(y_prob_1[:, 0, :, :, :], labeled_gt_set == 1)

            y_prob_2 = torch.sigmoid(l_outputs_np_2)
            y_all_2 = torch.sigmoid(model_2(volume_batch))
            y_sig_2_u = torch.sigmoid(u_outputs_np_2)

            loss_seg_2 = CE(y_prob_2.squeeze(1), labeled_gt_set.float())
            loss_seg_dice_2 = dice_loss(y_prob_2[:, 0, :, :, :], labeled_gt_set == 1)
            ...
            g_critic_segs_1_2 = torch.sigmoid(critic_1(labeled_gt_set.unsqueeze(1).float()))
            ...
            g_critic_segs_2_2 = torch.sigmoid(critic_2(labeled_gt_set.unsqueeze(1).float()))
