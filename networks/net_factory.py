...
from networks.TEM3D import TEM3D
from networks.TaylorEcho3DSeg import TaylorEcho3DSeg
...
def net_factory(net_type="unet", in_chns=1, class_num=4, mode = "train"):
    ...
    elif net_type == "tem3d" and mode == "train":
        net = TEM3D(n_channels=in_chns, n_classes=class_num, normalization='batchnorm', has_dropout=True).cuda()
    elif net_type == "tem3d" and mode == "test":
        net = TEM3D(n_channels=in_chns, n_classes=class_num, normalization='batchnorm', has_dropout=False).cuda()
    elif net_type == "taylorecho" and mode == "train":
        net = TaylorEcho3DSeg(n_channels=in_chns, n_classes=class_num, normalization='batchnorm', has_dropout=True).cuda()
    elif net_type == "taylorecho" and mode == "test":
        net = TaylorEcho3DSeg(n_channels=in_chns, n_classes=class_num, normalization='batchnorm', has_dropout=False).cuda()
